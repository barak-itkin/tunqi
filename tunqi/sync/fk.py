...
class BoundFK[S: Model, T: Model]:
    ...
    def get(self, *, fetch: bool = False) -> T | None:
        if self.pk is None:
            return None
        if not self.object or fetch:
            self.object = self.model.get(self.pk)
        return self.object

    def set(self, model: int | T | None) -> None:
        object = None
        if model is None:
        ...
        if self.source.pk:
            ...
            self.fk.source_model.update(pk=self.source.pk)(**update)
        self.pk = pk
        self.object = object
