...
if TYPE_CHECKING:
    from tunqi.sync.model import Model
...
class BoundM2M[S: Model, T: Model]:
    ...
    def exists(self, /, *, where: Expression | Query | None = None, **query: Any) -> bool:
        query[self._link] = self._assert_saved()
        return self.model.exists(where=where, **query)
    ...
    def count(
        ...
        **query: Any,
        ...
        query[self._link] = self._assert_saved()
        return self.model.count(distinct=distinct, where=where, **query)

    def get(self, /, *, where: Expression | Query | None = None, **query: Any) -> T:
        query[self._link] = self._assert_saved()
        return self.model.get(where=where, **query)
    ...
    def get_fields(
        ...
        **query: Any,
        ...
        query[self._link] = self._assert_saved()
        return self.model.get_fields(fields=fields, where=where, **query)
    ...
    def all(
        ...
        **query: Any,
        ...
        query[self._link] = self._assert_saved()
        return self.model.all(where=where, limit=limit, offset=offset, order=order, **query)
    ...
    def add(self, *models: T) -> int:
        ...
        return self.source._config.database.link(self.model._config.table_name, self.m2m.to, target_pks, [source_pk])

    def remove(self, *models: T) -> int:
        source_pk = self._assert_saved()
        target_pks = self._assert_models(models)
        return self.source._config.database.unlink(self.model._config.table_name, self.m2m.to, target_pks, [source_pk])
