...
async def test_database(db: Database, db_url: str, db_name: str) -> None:
...
async def test_default_database(db_url: str) -> None:
    db = Database(db_url)
...
async def test_active_database(db_url: str) -> None:
    db1 = Database(db_url)
    db2 = Database(db_url)
    db3 = Database(db_url)
    with db1:
...
async def test_invalid_dialect() -> None:
    with pytest.raises(
        RuntimeError,
        match=r"dialect 'mariadb' is not supported \(available dialects are sqlite, postgresql and mysql\)",
    ):
        Database("mariadb+asyncmy://localhost:1234/test")


async def test_serialization(db: Database) -> None:
