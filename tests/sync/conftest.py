...
from tunqi.sync import AuditEvent, Database, Row
...
def fields(items: list[Row]) -> set[Any]:
    return {tuple(item.values()) if len(item) > 1 else item.popitem()[1] for item in items}
